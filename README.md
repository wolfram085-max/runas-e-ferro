...
- Wiki pública separada do app do jogo.
...
- **App do jogo** (logado): `/game` e `/sala/demo`
- **Wiki pública**: `/wiki`
- **Admin**: `/admin`
...
- Scripts:
  - `npm run ingest:pdf` → lê `./assets/runas_e_ferro_livro_basico_v2.pdf` e cria `rule_pages`
  - `npm run index:rules` → cria índice para busca
  - `npm run seed:wiki` → cria páginas base da wiki (raças/bestiário)
  - `npm run media:generate` → gera placeholders de imagens em `/public/media`
